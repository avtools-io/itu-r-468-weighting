...
@pytest.mark.parametrize(
    "datum", ITU_R_468__FREQS_AND_EXP_VALS
)
def test_r468__against_itu_r_468_1khz_value_specs(datum):
    assert abs(datum.expected_db - r468(datum.frequency, datum.khz_option)) <= GLOBAL_DB_TOLERANCE
...
def test_r468__against_itu_r_468_1khz_value_specs_tolerances(datum):
    assert round(datum.expected_db - r468(datum.frequency, datum.khz_option), 1) >= datum.lower_tolerance
    assert round(datum.expected_db - r468(datum.frequency, datum.khz_option), 1) <= datum.upper_tolerance
